...
def predict_experiment(user, experiment_name):
    ...
    if file.filename != '':
        ...
        return jsonify({"result":{"dog":0.85, "cat":0.15}, "exp":experiment_name, "user": user})
...
def register_experiment(user, experiment_name):
    ...
    if user in dir_list_users:
        ...
        if experiment_name in dir_list_experiments:
            ...
            return jsonify({"user":user, "experiment":experiment_name,"response":"Experiment Successfully Created"})
    else:
...
def remove_experiment(user,experiment_name):
    ...
    if user in dir_list_users:
        ...
        if experiment_name in dir_list_experiments:
            ...
            return jsonify({"user":user, "experiment":experiment_name, "response":"Experiment Successfully Removed"})
        else:
